...
df['first_up_bar_low'] = np.nan  # Track where uptrend started
df['first_down_bar_high'] = np.nan  # Track where downtrend started

up_count = 0
down_count = 0
first_up_low = np.nan
first_down_high = np.nan
...
for i in range(len(df)):
    if df['upward'].iloc[i] == 1:
        ...
        down_count = 0
        if up_count == 1:
            first_up_low = df['low'].iloc[i]
        df.iloc[i, df.columns.get_loc('consecutive_up')] = up_count
        df.iloc[i, df.columns.get_loc('first_up_bar_low')] = first_up_low
    elif df['downward'].iloc[i] == 1:
        down_count += 1
        up_count = 0
        if down_count == 1:
            first_down_high = df['high'].iloc[i]
        df.iloc[i, df.columns.get_loc('consecutive_down')] = down_count
        df.iloc[i, df.columns.get_loc('first_down_bar_high')] = first_down_high
    else:
        up_count = 0
        down_count = 0
        first_up_low = np.nan
...
# ==========================================
# STRUCTURE-BASED STOP STRATEGY
# ==========================================
print("\n" + "="*60)
print("STRUCTURE-BASED STOP STRATEGY")
print("="*60)
print("\nStop Logic:")
print("  LONG: Stop at low of first up bar (trend structure break)")
print("  SHORT: Stop at high of first down bar (trend structure break)")
print("  Exit: Hold until trend reverses OR stop hit")

# Strategy parameters
TRANSACTION_COST = 0.0000
TAKE_PROFIT_PCT = 0.03  # 0.3% optional take profit
MAX_HOLD_BARS = 180  # Maximum hold time
...
for i in range(start_idx, len(df)):
    ...
    if position is not None:
        ...
        if position_type == 'long':
            # LONG exits
            # 1. Structure break: price goes below first up bar low
            if current_low <= position['structure_stop']:
                exit_reason = 'structure_break'
                exit_price = position['structure_stop']
            # 2. Take profit
            elif current_high >= position['take_profit']:
                exit_reason = 'take_profit'
                exit_price = position['take_profit']
            # 3. Trend reversal: downward bar appears
            elif df.loc[current_idx, 'downward'] == 1:
            ...
            # SHORT exits
            # 1. Structure break: price goes above first down bar high
            if current_high >= position['structure_stop']:
                exit_reason = 'structure_break'
                exit_price = position['structure_stop']
            # 2. Take profit
                ...
                exit_reason = 'take_profit'
                exit_price = position['take_profit']
            # 3. Trend reversal: upward bar appears
            elif df.loc[current_idx, 'upward'] == 1:
                exit_reason = 'trend_reversal'
            # 4. Max time
            elif bars_held >= MAX_HOLD_BARS:
                exit_reason = 'max_time'
        ...
        if exit_reason:
            ...
            if position_type == 'long':
                stop_distance = (position['entry_price'] - position['structure_stop']) / position['entry_price']
            else:
                stop_distance = (position['structure_stop'] - position['entry_price']) / position['entry_price']
            ...
            trades.append({
                ...
                'structure_stop': position['structure_stop'],
    ...
    if position is None:
        ...
        if df.loc[current_idx, 'long_signal'] == 1:
            structure_stop = df.loc[current_idx, 'first_up_bar_low']

            # Only enter if we have a valid structure stop
            if not pd.isna(structure_stop):
                position = {
                    'entry_idx': i,
        ...
        # Short entry
            ...
            structure_stop = df.loc[current_idx, 'first_down_bar_high']
...
if len(trades_df) > 0:
    print(f"\n{'='*60}")
    print("STRUCTURE-BASED STOP PERFORMANCE")
    print('='*60)
    ...
    if len(long_trades) > 0:
        ...
        print(f"  Avg stop distance: {long_trades['stop_distance_pct'].mean():.4f}%")
    ...
    if len(short_trades) > 0:
        ...
        print(f"  Avg stop distance: {short_trades['stop_distance_pct'].mean():.4f}%")
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            'SPY Price with Structure-Based Stops',
    ...
    for _, trade in trades_df.iterrows():
        ...
        fig.add_trace(
            go.Scatter(
                ...
                showlegend=False,
                hovertext=f"{trade['position_type'].upper()} entry<br>Stop: {trade['stop_distance_pct']:.2f}%"
            ),
            row=1, col=1
        )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=[trade['structure_stop'], trade['structure_stop']],
                mode='lines',
        ...
        fig.add_trace(
            go.Scatter(
                ...
                showlegend=False,
                hovertext=f"Exit: {trade['exit_reason']}<br>P&L: {trade['net_pnl']*100:.2f}%"
            ),
            row=1, col=1
        )
    ...
    fig.update_layout(
        title=f'Structure-Based Stop Strategy | P&L: {total_pnl*100:.2f}% | Win Rate: {winning/len(trades_df)*100:.1f}% | Trades: {len(trades_df)}',
        height=1600,
        showlegend=True,
    ...
    fig.update_xaxes(title_text="Stop Distance (%)", row=2, col=1)
    fig.update_yaxes(title_text="Count", row=3, col=1)
    fig.update_xaxes(title_text="Bars", row=3, col=1)
    fig.update_yaxes(title_text="Cumulative %", row=4, col=1)
    fig.update_yaxes(title_text="Count", row=5, col=1)

    fig.write_html('structure_based_stops.html')
    fig.show()

    trades_df.to_csv('structure_based_trades.csv', index=False)

    print("\nFiles saved:")
    print("  - structure_based_stops.html")
    print("  - structure_based_trades.csv")

    # Comparison
    print("\n" + "="*60)
    print("COMPARISON: Fixed % Stop vs Structure-Based Stop")
    print("="*60)
    print("\nFixed 0.12% Stop (Previous):")
    print("  - P&L: -0.57% (bidirectional)")
    print("  - Stop always 0.12% from entry")
    print("  - Many premature stop-outs")
    print("\nStructure-Based Stop (Current):")
    print(f"  - P&L: {total_pnl*100:.2f}%")
    print(f"  - Stop at first bar low/high")
    print(f"  - Avg stop distance: {trades_df['stop_distance_pct'].mean():.3f}%")
    print(f"  - Holds until reversal or structure break")
